...
let cookie,users,csrf_token,infoData,sendMsg,wyy,index = 0;
let shareMsg = "好歌推荐~~"
    ,commentMsg = "顶顶顶~~"
    ,reMsg = "感谢支持和意见~~"
    ,reCommentMsg = "感谢支持和意见~~";
...
if (!isPhone){
    wyy = require("./laoxinH_config").wangyi;
    shareMsg = wyy.shareMsg || shareMsg;
    commentMsg = wyy.commentMsg || commentMsg;
    reMsg = wyy.reMsg || reMsg;
    reCommentMsg = wyy.reCommentMsg || reCommentMsg;
    users = wyy.users;
...
!(async () => {
    for (let user of users) {
        index ++;
        let cookieFlag = await getCookie(user);
        if (!cookieFlag) continue;
        let userFlag = await getUserInfo();
        if (!userFlag) continue;
        // 获取任务列表
        await getTaskInfo();
        // 执行任务
        await runTask();
    ...
    let tempCk = user.cookie;
        if (!tempCk || tempCk == "") {
            ...
            if (reg_csrf.test(cookie) && reg_music_u.test(cookie)) {
                ...
                console.log("【错误❌】","账号 " + index + " 填写的cookie有误,参考格式：MUSIC_U=2da45fadcea83dbe492669abdc93166e004087dd3d78b6050a17a35e705925a4e6992f61dfe3f0151024f9e31; __csrf=e1f260e08c8b91351abd40466b14e;","请登录：https://music.163.com重新获取！");
            }
        }
        ...
        if (!user.phone || !user.password ) {
            console.log("【错误❌】","账号 " + index + " 登录失败","请到laoxinH_config.js填写账号和密码!");
            require("./sendNotify").sendNotify($.name,"账号【" + index + "】cookie格式化出错，且未填写账号密码，详情请查看脚本日志\r\n请按照要求正确填写cookie或者填写账号密码登录！");
            return false;
        }
        ...
        return new Promise(resolve => {
            $.post(opts,(err,res,data)=>{
                ...
                if (data != null && data.code == 200){
                    ...
                    // 发生错误通知
                    require("./sendNotify").sendNotify($.name,"账号【" + index + "】cookie格式化出错，且通过账号密码登录失败，详情请查看脚本日志\r\n\r\n请按照要求正确填写cookie或者检查账号密码是否错误\r\n如果账号密码正确但多次登录失败，则可能是异地登录问题，请使用cookie登录！");
                    resolve(false);
                }
            })
        })
...
async function getUserInfo(){
    ...
    if ((!cbData ||cbData.code == 301)){
        if (isPhone) {
            ...
            console.log("【通知】账号 " + index,"cooke已失效!","请打开https://music.163.com重新获取!");
            require("./sendNotify").sendNotify($.name,"账号【" + index + "】cookie已失效，请重新获取cookie！");
        }
        return false;
    }
    ...
    console.log("【通知📢】","账号【" + index + "】音乐人昵称: "+$.artistName,"当前账户云豆数量: "+ $.cbCount);
    return true;
}
    ...
    return new Promise(resolve => {
        $.post(getOpts(data,url),(err,res,data)=>{
            ...
            if (data != null && data.code == 200){
                ...
                $.logErr("失败");
                console.log("【错误❌】", "歌曲：" + reply.musicName,listElement.name + "的评论回复失败！", "原因：" + $.toStr(data));
                resolve(false);
            }
        })
    })
