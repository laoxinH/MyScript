...
let cookie ,csrf_token,infoData; // nodejs用户请直接将获取的cookie填写到双引号中
...
!(async () => {
    if (!await getCookie()) {
        console.log($.name,"无法获取cookie","执行结束!");
        $.done();
       return;
    }
    ...
    console.log("【通知📢】", "开始获取cookie, 当前环境 : " + (isPhone ? "手机端" : "nodejs"),"开始执行!");
        ...
        if (!(cookie && cookie != "")) {
            ...
            if (reg.test(cookie)) {
                ...
                console.log("【错误❌】","填写的cookie有误: 必须包含 __csrf=？？？？ 字段","请登录：https://music.163.com重新获取！");
            }
        }
